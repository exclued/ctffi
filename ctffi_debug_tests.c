...
#define LOG(fmt, ...) fprintf(stderr, "[ctffi-test] " fmt "\n", ##__VA_ARGS__)
#define FAIL(fmt, ...) do { LOG("FAIL: " fmt, ##__VA_ARGS__); return 1; } while (0)
...
static int compare_type(const char *label, ffi_type *a, ffi_type *b) {
    ...
    if (a->type == FFI_TYPE_STRUCT) {
        ...
        if (!a->elements || !b->elements)
            return a->elements == b->elements ? 0 : 1;
        while (a->elements[i] && b->elements[i]) {
            if (compare_type("struct element", a->elements[i], b->elements[i]))
                return 1;
            ++i;
        }
...
static int compare_cifs(const char *label, const ffi_cif *manual,
                        const ffi_cif *automatic) {
    ...
    if (manual->abi != automatic->abi ||
        ...
        manual->flags != automatic->flags)
        return 1;
    if (compare_type("return type", manual->rtype, automatic->rtype))
        return 1;
    for (unsigned i = 0; i < manual->nargs; ++i)
        if (compare_type("argument type", manual->arg_types[i], automatic->arg_types[i]))
            return 1;
    return 0;
}
...
static int dump_ctf_function(ctf_ffi_context_t *ctx, const char *name,
    ...
    if (info.ctc_argc) {
        ...
        if (!args)
            FAIL("out of memory inspecting %s", name);
...
static int test_point_distance(const char *path) {
    ...
    if (build_cif_from_ctf(&ctx, "point_distance", &automatic, &rtype,
                           &auto_args, &auto_nargs) != 0) {
        ctf_ffi_cleanup(&ctx);
        FAIL("automatic CIF construction failed");
    }
    dump_cif("automatic", &automatic);
    ...
    ctf_ffi_cleanup(&ctx);
    if (result != 25)
        FAIL("point_distance returned %d", result);
    return 0;
}
...
static int test_create_point(const char *path) {
    ...
    if (result->x != 10 || result->y != 20) {
        free(result);
        FAIL("create_point result mismatch");
    }
    free(result);
    return 0;
}
...
static int test_nested_struct(const char *path) {
    ...
    if (result->origin.x != 4 || result->origin.y != 6 || result->scale != 2.0) {
        free(result);
        FAIL("scale_point result mismatch");
    }
    free(result);
    return 0;
}
...
static int test_union(const char *path) {
    ...
    if (!result)
        FAIL("union_int call failed");
    LOG("union_int returned %d (expected 1234)", *result);
    if (*result != 1234) {
        free(result);
        FAIL("union_int result mismatch");
    }
    free(result);
    return 0;
}
